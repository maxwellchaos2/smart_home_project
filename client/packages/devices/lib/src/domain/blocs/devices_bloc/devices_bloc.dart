...
final class DevicesBloc extends Bloc<DevicesEvent, DevicesState> {
  ...
  DevicesBloc({
    ...
    on<_UpdateDevicesCapabilities>(
      (event, emit) async {
        await _repository.updateDevicesCapabilities(inputs: event.inputs);
      },
      transformer: concurrent(),
    );

    on<_UpdateDevicesProperties>(
      (event, emit) async {
        await _repository.updateDevicesProperties(inputs: event.inputs);
      },
      transformer: concurrent(),
    );
