...
import 'package:sleek_circular_slider/src/dashes_painter.dart';
...
class _SleekCircularSliderState extends State<SleekCircularSlider>
  ...
  void didUpdateWidget(SleekCircularSlider oldWidget) {
    if (oldWidget.angle != widget.angle &&
      ...
      _animate(widgetUpdated: true);
    }
    super.didUpdateWidget(oldWidget);
  }

  void _animate({bool widgetUpdated = false}) {
    ...
    _animationManager!.animate(
        ...
        angle: widget.angle,
        oldAngle: _currentAngle ?? widget.angle,
        oldValue: _currentAngle == null
            ? widget.initialValue
        ...
        valueChangedAnimation: ((double anim, bool animationCompleted) {
          ...
          setState(() {
            if (!animationCompleted) {
              ...
              _setupPainter(isFromAnimation: widgetUpdated);
              _updateOnChange();
            }
          });
        }));
  ...
  void _setupPainter(
      {bool counterClockwise = false, bool isFromAnimation = false}) {
  ...
  Widget _buildPainter({required Size size}) {
    return Opacity(
      ...
      child: CustomPaint(
        painter: DashesPainter(
          appearance: widget.appearance,
          minValue: widget.min,
          maxValue: widget.max,
          dashColor: const Color(0xFFE6E6E6).withOpacity(.5),
        ),
        child: CustomPaint(
          painter: _painter,
          child: SizedBox(
  ...
  Widget? _buildChildWidget() {
    ...
    final value = angleToValue(
        _currentAngle ?? widget.angle, widget.min, widget.max, _angleRange);
